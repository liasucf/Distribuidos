...
socket_servidor_tcp.send('portao'.encode())
...
class Portao():
    def __init__(self, socket_servidor_tcp):
        ...
        self.frame1_1_label = ttk.Label(self.frame11, text='Status do portao')
    ...
    def recieve_from_server(self):
        while True:
            ...
            if status != '':
                self.status = status
            print ("status do portao: ", self.status)
