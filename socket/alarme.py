...
socket_servidor_tcp.send('alarme'.encode())
...
class Alarme():
    def __init__(self, socket_servidor_tcp):
        ...
        self.frame1_1_label = ttk.Label(self.frame11, text='Status do alarme')
    ...
    def update_gui(self):
        if self.status == 'ativando alarme':
            text = 'Ativado'
        else:
            text = 'Desativado'
    ...
    def recieve_from_server(self):
        while True:
            ...
            if status != '':
                self.status = status
            print ("status do alarme: ", self.status)
